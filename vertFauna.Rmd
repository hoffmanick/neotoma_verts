...
num_points = length(point_vert_datasets[[1]])
num_poly = length(poly_vert_datasets[[1]])
...
The spatial precision of Neotoma data refers to how closely a site's location as designated in Neotoma maps on to the site's actual location. `r num_points` of the vertebrate datasets in Neotoma are point locations, while `r num_poly` are bounding boxes. According to the [Neotoma Manual](https://neotoma-manual.readthedocs.io/en/latest/tables_site.html#sites), “the lat-long box can be used either to circumscribe the areal extent of a site or to provide purposeful imprecision to the site location.” In other words, there is an ambiguity inherent to the use of bounding boxes in Neotoma. Some are for designating the full extent of a lake, while others are used for fuzzing the coordinates of sensitive sites. Adding to this confusion is the fact that different constituent databases have historically addressed spatial precision differently. FAUNMAP, which contains most (n=4851) of Neotoma's vertebrate data, has many sites that are designated as points but which actually are fuzzed to some extent. The original FAUNMAP database had metadata on the precision of its sites, but Neotoma no longer does. See this [page section](https://hoffmanick.github.io/eos/neoHomo.html#6_Are_the_coordinates_for_any_sites_in_Neotoma_fuzzed) for an attempt at matching Neotoma's FAUNMAP data to original FAUNMAP precision metadata.
...
These graphs show dataset-level temporal precision. For datasets with an age type of calendar years AD/BC, I converted to BP. The first is colored by database name, and the second is colored by the chronology type.

```{r collection-chron,fig.width=11,fig.height=6}


index = seq(length(all_distinct_chrons[[1]]))
...
all_distinct_chrons_mut = all_distinct_chrons_mut %>% 
  ...
  older < 11625 ~ "Holocene",
  older >= 200000 & younger <= 200000 ~ "Early Pleistocene",
  older >= 11625 & younger <= 11625 ~ "Late Pleistocene",
  younger > 11625 & younger <= 200000 & older > 11625 & older <= 200000 ~ "Late Pleistocene",
  younger > 200000 & younger <= 2580000 & older > 200000 & older <= 2580000 ~ "Early Pleistocene",
  older >= 2580000 & younger <= 2580000 ~ "Pliocene",
  younger > 2580000 & younger <= 5333000 & older > 2580000 & older <= 5333000 ~ "Pliocene",
  older >= 5333000 & younger <= 5333000 ~ "Pliocene",
  younger > 5333000 & younger <= 23030000 & older > 5333000 & older <= 23030000 ~ "Pliocene",
)) %>% dplyr::filter(!is.na(younger))


all_distinct_chrons_mut$period <- factor(all_distinct_chrons_mut$period, levels = c("Holocene", "Late Pleistocene", "Early Pleistocene","Pliocene"))

ds_chron = ggplot(all_distinct_chrons_mut) +
  geom_segment(mapping=aes(x=younger,
                           xend=older,
                           y=index,yend=index,
                           color=databasename,
...
And this graph shows sample-level temporal precision. 


```{r analysis-chron,fig.width=11,fig.height=6}
...
all_distinct_chrons$older = as.numeric(all_distinct_chrons$older)
all_distinct_chrons$younger = as.numeric(all_distinct_chrons$younger)

dcc = dcc %>% left_join(all_distinct_chrons,by=join_by(chronologyid,older,younger)) %>% distinct(sampleid,.keep_all = TRUE) %>%
  dplyr::mutate(older = case_when(
    agetype == "Calendar years AD/BC" ~ 1950 - older,
    TRUE ~ older
  )) %>% 
...
dcc$period <- factor(dcc$period, levels = c("Holocene", "Late Pleistocene", "Early Pleistocene","Pliocene"))
...
The below table shows all the taxa observed in vertebrate fauna datasets ranked by the number of datasets in which they occur. The graph below that is a rank-abundance curve.

```{r ranks}

n_datasets = all_dat_df %>%  distinct(taxonid,variablename,datasetid) %>% group_by(taxonid,variablename) %>% count() %>% arrange(desc(n))

datatable(n_datasets,rownames=FALSE)
rank = seq(length(n_datasets[[1]]))

n_datasets = n_datasets %>% cbind(rank)
names(n_datasets)[[4]] = "rank"

ggplot(n_datasets) +
  geom_line(mapping=aes(x=rank,y=log(n))) +
  theme_bw() +
  labs(x="Taxonomic Abundance Ranking", y="Number of IDs (logarithmic)")
```

## Taxonomic Precision

I harmonized the data from vertebrate fauna datasets in Neotoma with a table that Val and Jessica made which also described the taxonomic precision of the identification. That table captured 567 of the 2721 taxa described in Neotoma vertebrate fauna datasets (20.83%), and 32,965 of the 79,069 of the observations in the datasets (41.70%). 
...
unc_converter = unc_converter %>% arrange(desc(values))
datatable(unc_converter,rownames=FALSE, options = list(
    pageLength = 12
  ))
...
plot_vdf = ggplot(vdf) +
  ...
  scale_y_continuous(breaks=c(1,2,3,4),labels=c("Species","Genus","Family","Order"))
...
The Mammal Diversity Database won't match taxa starting with ? or cf., or with sp. in the name. It also won't match names that are cf. with respect to genus or which are two identificatons joined by a slah. I removed those precision markers from the taxa names for the matching process. (I kept the presence or absence of those markers in a new column though, so it's not lost, but I haven't used that information in the below visualizations.)

I ended up matching 52,477 observations out of 79,069, and 1325 out of 2721 taxa. There's a bias toward missing all the extinct species, so you don't see the pilosa or cingulates from the Americas that you might otherwise. There are also a few nomenclature issues, some of which are amenable to similar adjustments to the certainty marker edits I did above, and some of which reflect synonymy issues.

I surveyed 500 of the unmatched names, and I found that about half were unmatched because nonmammal; about a third were extinct mammals, about 10% were nomenclature issues of one kind or another, and the last 5% I couldn't figure out why they didn't match.

For the sake of simplicity, I ignore cf., sp., etc. in my table below. (But I retain that information in the dataframe.)
...
harm7 = separate(all_dat_df,variablename2,into=c("var1","var2"),sep="/") %>%
  mutate(
    var2 = if_else(
      str_detect(var2, "^[A-Z]\\."),                               # if var2 starts with "X."
      str_replace(var2, "^[A-Z]\\.", str_extract(var1, "^[^ ]+")), # replace "X." with first word of var1
      var2
    )
  )

harm71 = harm7 %>% dplyr::select(var1,cf_or_q,sp_spp,databasename)
names(harm71)[1] = "var2"
harm72 = harm7 %>% dplyr::select(var2,cf_or_q,sp_spp,databasename)
harm7 = harm71 %>% rbind(harm72) %>% drop_na(var2)

harm7 = harm7 %>%
  mutate(
    cf_species = str_detect(var2, "cf\\. [A-Z]\\."),                          # TRUE if cf. X. is present
    var2 = str_replace(var2, "cf\\. [A-Z]\\.[ ]?", "")                      # remove cf. X. and the following space
  )

harm3 = harm7 %>% left_join(taxaharm2,by=join_by("var2"=="MDD_original_combination")) %>% drop_na(MDD_order) %>% select(var2,MDD_genus,MDD_order,MDD_family,cf_or_q,sp_spp,cf_species,databasename) %>% dplyr::mutate(certainty="species")

harm3_na = harm7 %>% left_join(taxaharm2,by=join_by("var2"=="MDD_original_combination")) %>% dplyr::filter(is.na(MDD_order))

harm3_distinct = harm3_na %>% distinct(var2)

mdd_genera = taxaharm2 %>% distinct(MDD_genus,.keep_all=TRUE)

harm4 = harm7 %>% dplyr::filter(var2 %in% harm3_distinct$var2) %>% left_join(mdd_genera,by=join_by("var2"=="MDD_genus")) %>% drop_na(MDD_order)  %>% dplyr::mutate(MDD_genus = var2) %>% select(var2,MDD_genus,MDD_order,MDD_family,cf_or_q,sp_spp,cf_species,databasename) %>% dplyr::mutate(certainty="genus")


harm4_na = harm7 %>% dplyr::filter(var2 %in% harm3_distinct$var2) %>% left_join(mdd_genera,by=join_by("var2"=="MDD_genus")) %>% dplyr::filter(is.na(MDD_order)) 

harm4_distinct = harm4_na %>% distinct(var2)


mdd_families = taxaharm2 %>% distinct(MDD_family,.keep_all=TRUE)

harm5 = harm7 %>% dplyr::filter(var2 %in% harm4_distinct$var2) %>% left_join(mdd_families,by=join_by("var2"=="MDD_family")) %>% drop_na(MDD_order)  %>% dplyr::mutate(MDD_family = var2) %>% select(var2,MDD_genus,MDD_order,MDD_family,cf_or_q,sp_spp,cf_species,databasename) %>% dplyr::mutate(certainty="family")


harm5_na =  harm7 %>% dplyr::filter(var2 %in% harm4_distinct$var2) %>% left_join(mdd_families,by=join_by("var2"=="MDD_family")) %>% dplyr::filter(is.na(MDD_order))

harm5_distinct = harm5_na %>% distinct(var2)


mdd_orders = taxaharm2 %>% distinct(MDD_order,.keep_all=TRUE)

harm6 = harm7 %>% dplyr::filter(var2 %in% harm5_distinct$var2) %>% left_join(mdd_orders,by=join_by("var2"=="MDD_order")) %>% drop_na(MDD_species)  %>%
  dplyr::mutate(MDD_order = var2) %>% select(var2,MDD_genus,MDD_order,MDD_family,cf_or_q,sp_spp,cf_species,databasename) %>% dplyr::mutate(certainty="order")


harm6_na =  harm7 %>% dplyr::filter(var2 %in% harm5_distinct$var2) %>% left_join(mdd_orders,by=join_by("var2"=="MDD_order")) %>% dplyr::filter(is.na(MDD_species))

harm6_distinct = harm6_na %>% distinct(var2)
...
specharm = all_harm %>% group_by(MDD_order) %>% dplyr::filter(!certainty %in% c("order","family","genus")) %>% distinct(var2) %>% count() %>% arrange(desc(n))
